...
トップページで示した将来像を具体化するページ。本文は手書きの静的HTMLとし、`<!-- reflections:start -->`と`<!-- reflections:end -->`の間にあるResonancesだけをビルド時に更新する。

Reflectionのfrontmatterに`aspirations: <slug>`を指定すると、対応するAspiration詳細にカードが表示される。AspirationのslugはHTMLファイル名から拡張子を除いた値を用いる。
...
新規原稿はObsidian Vault側で作成する。StyleとConceptは `STYLE_DIR` と `CONCEPT_DIR` を分けて指定し、frontmatterの `section`、`slug`、`sort_order`、`is_published` を読む。Reflectionは `REFLECTIONS_DIR` を指定し、frontmatterの `slug`、`title`、`date`、`description`、`styles`、`concepts`、`aspirations` を読む。Reflectionの接続先は `styles`、`concepts`、`aspirations` にslugをカンマ区切りで指定する。従来の`tags`もStyle・Conceptの接続先として読み込む。

`STYLE_DIR` と `CONCEPT_DIR` が未設定の場合は、後方互換として `CONCEPTS_DIR`、さらに `content/concepts/` を読む。`REFLECTIONS_DIR` が未設定の場合は `content/reflections/` を読む。`content/` はローカルフォールバックであり、編集上の正本ではない。
...
- Aspiration詳細ページの本文（Resonances生成範囲を除く）
- 404ページ
- コンセプトページの共通テンプレート

ただし、トップページ内のコンセプト生成範囲、`concepts/*.html`、`style/*.html`、`reflection/posts/*.html`、Aspiration詳細内のResonances生成範囲は直接編集しない。
